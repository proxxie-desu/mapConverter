...
// Guess I might as well use Jquery if I had to include it anyway
$(document).ready(function() {
    // Make stupid Bootstrap files button change
    document.getElementById("inputFiles").addEventListener('change', function() {
        let inFiles = document.getElementById("inputFiles").files;
        if (inFiles && inFiles.length > 1) {
            document.getElementById("inFilesLabel").innerHTML = inFiles.length + " Files";
        } else if (inFiles) {
            document.getElementById("inFilesLabel").innerHTML = inFiles[0].name;
        }
    });
});
